<div class = "lihat-produk-lain"><h3>Testimoni</h3></div>	
	...
	<form action = "simpan-testimoni.php" method = "POST">
		...
		<?php
		$panggil = mysqli_query("SELECT kd_brg FROM barang WHERE kd_brg='$_GET[kd]'", $konek);
	...
	<table border ="1" class = "tabel-testimoni">
		<tr>
			...
			$tambah = mysqli_query ($tampil) or die ("Gagal".mysqli_error());
			$dilihat = mysqli_num_rows ($tambah);
			
			$awal = 0;
			$max = 4;
			while ($row = mysqli_fetch_array($tambah) and ($awal <= $max))
			{				
				$a = $row['no_testi'];
				$b = $row['nama'];
				$c = $row['kota'];
