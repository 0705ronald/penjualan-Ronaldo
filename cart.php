...
if(isset($_GET['beli'])){
	$sid = session_id();
	$query = mysqli_query("SELECT * from barang where kd_brg = '".$_GET['beli']."' ");
	...
	if($row['jumlah_brg'] < 1){
		...
		$sql = mysqli_query("SELECT * from transaksi_temp WHERE kd_brg='".$_GET['beli']."' AND no_session='$session_id'");
		$ketemu = mysqli_num_rows($sql);

		if($ketemu >= 1){
			mysqli_query("UPDATE transaksi_temp SET jumlah_brg = jumlah_brg + 1 WHERE no_session = '$session_id' AND kd_brg='".$_GET['beli']."' ");
		}
		else {
			// isi tabel
			mysqli_query("INSERT INTO transaksi_temp values ('','".$_GET['beli']."','$session_id','1','$tanggal','$jam')");
		}
	}
...
function cart(){
	...
	$query 		= mysqli_query("SELECT * FROM barang INNER JOIN transaksi_temp ON barang.kd_brg = transaksi_temp.kd_brg WHERE no_session = '$session_id'");
