...
require "konek_db.php";
		...
		$query = mysqli_query ($tampil) or die ("Gagal".mysqli_error());
...
<div id = "sitecontainer-cek-transaksi">
		<div class = "menu-kiri">
			...
			<div class = "menu-kiri-3">
				<ul><h4 class = "menu-kiri-terbaru">Produk Terbaru</h4>
					...
					$awal = 0;
					$max = 5;
					while ($row = mysqli_fetch_array($query) and ($awal <= $max))
					{
						$a = $row['nama_gambar'];
	...
	<?php
		...
		$a = $_POST['notagihan'];
		$tampil = mysqli_query("select * from transaksi WHERE no_tagihan='$a'");
		...
		if($a == ""){
			...
			echo "<p style = 'margin:10px;'>Terima kasih Anda sudah berbelanja di Toko Online kami. Dan berikut ini adalah data transaksi anda.</p>";
				$ambil = mysqli_query("SELECT * FROM transaksi WHERE no_tagihan ='$a'");
