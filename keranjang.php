...
<?php
	...
	$tampil = mysqli_query("SELECT barang.kd_brg, barang.nama_brg, barang.harga_barang, transaksi_temp.jumlah_brg FROM barang INNER JOIN transaksi_temp ON barang.kd_brg = transaksi_temp.kd_brg WHERE transaksi_temp.no_session = '$session_id' ");
...
?>
	...
	if(isset($_GET['tambah'])){
		...
		$kode_brg = mysqli_real_escape_string($_GET['tambah']);

		$query = mysqli_query("SELECT 	barang.jumlah_brg AS jml_brg, 
		...
		if ($data['jumlah_brg'] >= $data['jml_brg'] ) {
			...
			mysqli_query("UPDATE transaksi_temp SET jumlah_brg = jumlah_brg + 1 WHERE kd_brg = '$kode_brg' ");
		...
		$query = mysqli_query("SELECT * FROM transaksi_temp WHERE kd_brg ='".$_GET['kurangi']."' AND no_session = '".$id_session."' ");
		$data = mysqli_fetch_array($query);


		if ($data['jumlah_brg'] > 1) {
			mysqli_query("UPDATE transaksi_temp SET jumlah_brg = jumlah_brg - 1 WHERE kd_brg = '".$_GET['kurangi']."' ");
		}else{						
			mysqli_query("DELETE FROM transaksi_temp WHERE kd_brg = '".$_GET['kurangi']."' AND no_session = '".$id_session."' ");
		}

		echo "<meta http-equiv='refresh' content='0;keranjang.php' />";
	}elseif(isset($_GET['hapus'])){
		mysqli_query("DELETE FROM transaksi_temp WHERE kd_brg = '".$_GET['hapus']."' ");
		echo "<meta http-equiv='refresh' content='0;keranjang.php' />";
	}
	?>
